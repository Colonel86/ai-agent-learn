...
```
                面试复习总包(本目录,10 章 = 可背诵的"机制+取舍+QA")
                          │  每章 §7 回链
        ┌─────────────────┼──────────────────────┐
        ▼                 ▼                        ▼
  ../1.md 五层心智   ../3.md context 数据结构   ../../roadmap/agent-selection/*
  (L0~L5 + 横切带)    (按访问模式分层选结构)      (分层架构选型决策包,架构师资产)
  —— 权威源,本包是    —— 本包 04/05/08 的         —— 本包每章对照的"已有资产",
     它在"面试问答"      数据结构落点               面试差异化亮点(见 §6)
     维度的展开
```
