...
**🧩 接入 Spec-Kit** → 统一见本文 §七(可复制 prompt 块含配置版本化输入项)。
...
```
请用 agent/skills/agent-selection/5-observability-eval.md 为本 Agent 选可观测平台 + eval 方案 + 配置版本化方案。
- 现有栈:<LangChain/LlamaIndex/PydanticAI/裸SDK…>
- 是否要自托管/OSS:<…>  是否 RAG:<…>  是否多步 agent(需轨迹评估):<…>  合规/数据是否出域:<…>
- 配置版本化(子决策 3):谁改 prompt <只工程/含产品·运营>;要不要运行时热更/灰度 <…>;
  要版本化的配置:prompt / 工具 schema / model id / 推理参数(temperature·seed) / 检索配置 <列出>
请分别给:① 可观测平台 推荐+备选+理由(先答"trace 如何产生",见子决策 0);
② eval 库+方法(类型/节奏/层级)推荐+备选+理由+代价;
③ 配置版本化方案 推荐+备选+代价(默认 prompt 进 git + 钉 model id)+ 与 eval 联动的回归/回滚/灰度 + 升级触发条件。
具体平台/价格现查官网,别写死过期版本。
```
