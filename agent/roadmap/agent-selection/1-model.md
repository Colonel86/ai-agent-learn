...
```
单模型直连(各节点固定档位)                       ← 默认,先到这,别预支复杂度
  │ 成本爆了 / 简单请求占多数
  ├─► 加级联:轻量级先试 + 升档判据(置信度/校验器)
  │ 要跨厂商 / 要 fallback / 要集中成本观测
  ├─► 上 LLM 网关(单厂商够稳→LiteLLM 自托管;想快横评→OpenRouter)
  │ 要集中治理(缓存/护栏/预算)
  └─► 企业网关(Portkey 等),与护栏层(7-safety-guardrails.md)合流
```
