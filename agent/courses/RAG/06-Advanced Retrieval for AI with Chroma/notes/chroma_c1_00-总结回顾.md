...
- **诊断工具 UMAP**：把 384 维 embedding 投影到 2D，肉眼看 query（🔴）、召回文档（🟢）、整个数据集（灰）的几何关系。Distractor 一眼可见。
  - 几何直觉三要点：① Query 在**稠密区** → 邻居紧凑相关；② 在**稀疏边缘** → 结果四面八方、杂乱；③ **超出数据集范围** → 仍被强制返回 Top-K，全是噪声。
  - ⚠️ Distractor 的真实危害是一条链：**相似 ≠ 相关 → Distractor 入场 → 污染 RAG 上下文 → LLM 输出次优 → 用户说不清哪错 → 开发者调试地狱**。最坑的不是错本身，而是错得隐蔽、难以归因。
...
**几何视角（一图理解两种做法）**：Query = 嵌入空间中的**一个点**。
- 原始 Query → 一个点
- 假设答案增强 → **移动**这个点到更合适的位置
- 多 Query 增强 → **克隆**成多个点，覆盖更大区域
...
> 策略：先用 Bi-Encoder 粗筛成 10–100 个候选，再用 Cross-Encoder 精排。
