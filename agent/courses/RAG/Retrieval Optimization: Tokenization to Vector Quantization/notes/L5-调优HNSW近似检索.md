...
```mermaid
flowchart TB
    subgraph L2["Layer 2 (最稀疏)：少量向量、长跳边"]
      A2["●"] -.->|"长跳边"| B2["●"]
    end
    subgraph L1["Layer 1"]
      A1["●"] --- B1["●"] --- C1["●"] --- D1["●"]
    end
    subgraph L0["Layer 0 (最稠密)：全部向量都在底层"]
      A0["● ● ● ● ● ● ● ● ● ● ● ● ● ● ●"]
    end
    L2 -->|"层间边（同一向量在不同层的连接）"| L1
    L1 --> L0
```
...
```mermaid
flowchart TB
    A["query 向量化（同一 embedding 模型）"]
    A --> B["从顶层进入，找 ef 个最近点"]
    B --> C["顺层间边下沉到下一层，只看「上一步选中点 + 其直接邻居」，再选 ef 个"]
    C --> D["逐层重复，作用域不断收窄"]
    D --> E["到底层取 top-k 返回（k 通常 &lt; ef）"]
```
