...
```
input_folder/ (uploadA..E, 类型未知)
      │
 ① Parse(split="page")           逐页解析
      │  取首页 markdown
 ② Extract + DocType schema       判定文档类型（分类）
      │
 ③ 按类型选对应 Pydantic schema
      │  Extract 抽字段 + grounding
 ④ 汇总 DataFrame + 跨文档校验    姓名一致?年份一致?资产合计
```
