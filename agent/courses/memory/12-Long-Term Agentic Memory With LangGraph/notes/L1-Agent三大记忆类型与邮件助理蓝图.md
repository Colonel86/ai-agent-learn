...
**30 秒版**：Agent 长期记忆是两个正交决策——**记什么**：Semantic（事实→向量库条目）/ Episodic（历史案例→prompt 里的 few-shot）/ Procedural（规则→System Prompt 本身，可被后台 agent 自动优化）；**何时更新**：Hot Path（即时读写，立刻生效但增延迟）vs Background（helper agent 异步更新，主 agent 专注但非即时）。选型看场景的即时性要求和延迟容忍度，不是全都上。

被追问时的展开点：

- **类比**（帮记忆）：Semantic=课本知识/别人生日；Episodic=去迪士尼那次经历；Procedural=怎么骑车
- **选型规则**：刚说出口必须马上可用 → Hot Path；历史汇总/prompt 长期演进 → Background

加分点：

- **"三个自问"是可迁移模板**：要不要学更好的指令(Procedural)？从过去案例学(Episodic)？记住人/地/物(Semantic)？——比背名词更体现架构能力
- **两个维度正交，别混谈**：同一类记忆既可 Hot Path 也可 Background（Semantic 偏好走 Hot Path，历史汇总类 Semantic 更适合 Background）
- **Procedural 最反直觉也最有价值**：System Prompt 是可演化的运行时数据，后台 agent 按反馈改写——"agent 自我改进"的朴素形态

> 关联：`../../../frameworks/11-AI Agents in LangGraph/notes/L05-持久化与流式输出.md`（记忆落到持久化底座）、`../../../../skills/agent-selection/`（记忆选型矩阵）。
