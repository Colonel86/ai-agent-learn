...
```
┌─► ① 处理 completion 当前状态：深度理解词与词的关系、文本整体含义
│   ② 遍历词表全部 token（数万~数十万），逐个计算"它出现在下一位"的概率
│      → 得到一个覆盖整个词表的概率分布
│      （shining 80%，rising 次之，exploding/snoring 也有微小概率）
│   ③ 从该分布中随机抽取下一个 token
└── ④ 把新 token 接到 completion 末尾，回到 ①，直到发出"完成"信号返回用户
```
...
```
prompt ──► ① 处理 prompt，理解其底层含义
              ▼
           ② 用这个理解去搜索文档 index
              ▼
           ③ 返回知识库中判定"与 prompt 最相关"的文档
```
...
```
返回太多 ◄──────────────────┼──────────────────► 返回太少
把整个知识库都给你：          理想点：完美排序 +      只取 top-1：
相关的都在，但淹没在          恰好正确的返回数量      第 2/3/4 名里的
无关信息的大山里；                                   有价值信息被漏掉
prompt 变贵甚至撑爆
context window
```
