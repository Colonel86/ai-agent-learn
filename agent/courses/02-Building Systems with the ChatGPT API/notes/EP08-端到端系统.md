...
```mermaid
flowchart TB
    U["用户输入 (user_input)"] --> S1["Step 1: Moderation API（输入侧）"]
    S1 -->|flagged| R1["返回拒绝消息，终止"]
    S1 -->|pass| S2["Step 2: 提取产品类别 & 名称列表<br/>utils.find_category_and_product_only()<br/>utils.read_string_to_list()"]
    S2 --> S3["Step 3: 查询产品详情（RAG 检索）<br/>utils.generate_output_string()<br/>若无匹配产品 → product_information 置为空串"]
    S3 --> S4["Step 4: 生成回答<br/>system_message + 用户消息 + 产品信息 + 对话历史<br/>get_completion_from_messages(all_messages + messages)"]
    S4 --> S5["Step 5: Moderation API（输出侧）"]
    S5 -->|flagged| R2["返回拒绝消息，终止"]
    S5 -->|pass| S6["Step 6: 模型自评（回答是否充分？）<br/>把 user_input + final_response 交给模型评估"]
    S6 --> S7["Step 7: 路由决策"]
    S7 -->|自评 Y| Y["返回 final_response 给用户"]
    S7 -->|自评 N| N["返回转人工消息"]
```
...
```mermaid
flowchart TB
    A["调用方持有 context（all_messages）"] --> B["每次调用 process_user_message(user_input, context)"]
    B --> C["函数返回 (response, updated_context)"]
    C --> D["调用方更新 context，下次传入"]
    D -.-> A
```
