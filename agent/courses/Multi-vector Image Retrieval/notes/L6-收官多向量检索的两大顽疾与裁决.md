...
```
L1 ColBERT (文本)          概念地基：late interaction + MaxSim
   │  同一套思想迁移到图像
L2 ColPali (图像)          页面截图 → 每页 ~1031 个 patch token 向量
   │  多向量 = 数据爆炸
L3 优化                    量化(4×~32×省显存) + 池化(向量数减半)
   │  省了显存，仍建不了 HNSW
L4 MUVERA                  变长多向量 → 定长 FDE → 解锁 HNSW + 两阶段
   │  零件齐了
L5 多模态 RAG              ColPali/MUVERA 检索 + GPT-4o 看图生成，无 OCR
```
