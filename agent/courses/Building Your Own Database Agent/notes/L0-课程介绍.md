...
```
业务用户（自然语言）──▶ Database Agent（LLM 编排）──▶ 数据源
        「7 月德州住院多少人？」        │                 CSV / SQLite / SQL DB
                                        ▼
                            自动生成动作序列（SQL / function call）
                                        ▼
                                  取回数据 + 解释推理过程
```
...
```
L1 连通 Azure OpenAI + LangChain（Hello World 翻译）
     │  加数据源
     ▼
L2 pandas DataFrame Agent 问 CSV（RAG on 表格）
     │  换数据源
     ▼
L3 SQL Agent 问 SQLite（NL→SQL，暴露查询轨迹）
     │  换取数方式（引出后半部分）
     ▼
L4 Function Calling（不暴露 SQL）  L5 Assistants API（带状态/记忆）
```
