...
```mermaid
flowchart LR
    T0["temperature = 0"] --> A0["总是选最高概率词"] --> R0["输出稳定可复现<br/>✅ 推荐：提取、分类、结构化任务、生产环境"]
    T7["temperature = 0.7"] --> A7["引入随机性，偶尔选概率较低的词"] --> R7["输出有变化<br/>✅ 推荐：邮件写作、内容改写、创意生成"]
    T1["temperature = 1.0"] --> A1["最大随机性"] --> R1["✅ 推荐：头脑风暴、故事创作"]
```
