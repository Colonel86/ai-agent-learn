...
```
demo（1 个输入）✓ → 10 个输入 ✓ → 100 个（生产）✗ → 1000 个 ✗✗
                                  "80% 可靠" ≈ 不可用
```
...
```
CrewAI training → guardrails → CrewAI testing → （真有必要才）fine-tune 模型
        └────────── 全部反哺进 agent 的 memory ──────────┘
```
...
输入：对方 email
   │
   ├─ Research agent ──→ 背景调研（工作经历、公司、发表文章）
   ├─ Email agent ─────→ 检索与此人的历史邮件往来
   │
   └─→ 最终 meeting report：带着完全准备走进会议
```
...
```
概念（concept）──────很宽的沟──────▶ 原型（prototype）──────更宽的沟──────▶ 生产（production）
   idea 极简单                        跑通用例                          + 制衡、检查、监控、observability
                        ↑ 若不从一开始就想清楚，后面必出问题
```
