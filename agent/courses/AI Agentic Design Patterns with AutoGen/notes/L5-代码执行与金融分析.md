...
```
code_executor_agent ──任务描述──▶ code_writer_agent (GPT-4 Turbo)
                    ◀─计划+代码── ①拟计划：yfinance 取数 → 算涨幅 → matplotlib 画图 → 存盘
 [人类审查代码：按 Enter = 跳过反馈、走 auto-reply 放行；也可键入修改意见]
code_executor_agent 执行代码 ──stdout/结果──▶ code_writer_agent
                    ◀─"执行成功，图已生成" + TERMINATE──
 [人类键入 exit 结束对话]
Image(os.path.join("coding", "ytd_stock_gains.png"))   # notebook 里查看产物
```
