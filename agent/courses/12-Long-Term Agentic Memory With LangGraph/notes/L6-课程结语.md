...
```mermaid
flowchart TB
    L0["L0 课程介绍 ← 三类记忆的概念框架"]
    L1["L1 邮件助理蓝图 ← 把三类记忆映射到具体应用"]
    L2["L2 Baseline Email Agent ← LangGraph 双层架构（Triage + Response）"]
    L3["L3 + Semantic Memory ← Hot Path：用户事实即时读写"]
    L4["L4 + Episodic Memory ← Few-Shot：从案例学习决策"]
    L5["L5 + Procedural Memory ← 自我演化：prompt 自动优化"]
    L6["L6 课程结语 ← 本课"]
