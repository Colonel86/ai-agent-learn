"""ByteChapters 工单机器人 —— L5 全流程红队评估的目标应用。

这是课程原版 helpers/byte_chapters.py 的移植。和 zb_app.py 不同,它不是问答型 RAG,
而是一个**能动手改数据的 Agent**:六个工具可以查订单、取消订单、判定退款资格、
执行退款、升级人工。红队价值就在这——攻击面从"说错话"升级成"做错事"。

移植改动同样只有两类:llama-index 0.9 → 0.14 的命名空间迁移,以及模型接入换成
本地栈(见 local_stack.py)。工具、系统提示词、订单数据全部保持原样。
"""
...
from helpers.data.byte_chapters.data import BC_ORDERS, get_templates_index


def _new_orders_store() -> pd.DataFrame:
    df = pd.DataFrame(BC_ORDERS).set_index("order_id")
    # 退款时会往 notes 列写原因。pandas 3.0 起用 .loc 给不存在的列赋值不再自动建列,
    # 所以先把它建出来。
    df["notes"] = ""
    return df


_orders_store = _new_orders_store()
...
def get_retriever():
    """处置模板检索器(懒加载,原版是模块级即时构建)。"""
    global _retriever
    if _retriever is None:
        _retriever = get_templates_index().as_retriever()
    return _retriever
...
class ByteChaptersAgent:
    def __init__(
        ...
        self._llm = llm or local_stack.get_llm(temperature=0)
    ...
    def chat(self, message: str) -> str:
        ...
        context = (
            ...
            + "\n---\n".join(n.text for n in get_retriever().retrieve(message))
        )
        ...
        if tool_calls:
            # 原版这里是"每处理一个 tool_call 就立刻再调一次 LLM"。模型一次只返回
            # 一个工具调用时看不出问题,一次返回多个就会构造出非法的消息序列:
            # 带 tool_calls 的 assistant 消息后面必须紧跟**每一个** tool_call_id
            # 对应的 tool 消息,少一条都不行。OpenAI 当年宽容地放过了,DeepSeek
            # 直接 400("insufficient tool messages following tool_calls message")。
            # 正确做法是先把所有工具调用的结果补齐,再做一次收尾生成。
            for tool_call in tool_calls:
                chat_history.append(self._call_function(tool_call))
