...
```python
def run_query_gen_pipeline(gold_queries):
...
results = run_query_gen_pipeline(gold_queries)
```
...
def run_eval(dataset, args):
    results = run_evaluation_pipeline(dataset, args)
    print("Total results:", len(results))
    return results


def run_evaluation_pipeline(dataset, args):
    results = EvaluationPipeline(args).call(dataset, type="eval")
    return results
...
def run_evaluation_pipeline(dataset, args):
    results = EvaluationPipeline(args).call(dataset, type="large eval")
    return results
