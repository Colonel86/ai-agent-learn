...
```
① 搭起应用 + Prompt Engineering（含 self-reflection）
        ↓ 不够准
② 严格 Evaluation：量化知道「行不行」+「下一个准确率前沿在哪」
        ↓ 还不够准
③ 用 LLM 生成 fine-tuning 数据集 → fine-tune / memory tune
        ↓
   迭代（evaluation ↔ data generation ↔ tuning 三步循环）
```
