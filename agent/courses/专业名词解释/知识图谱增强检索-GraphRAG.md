...
```
向量 RAG：  query ──► 找语义最像的几段话            (靠"距离")
KG 增强：    把文档变成 [A公司]—CEO是→[张三]—投资了→[B公司]—主营→[芯片]
            query ──► 识别实体 → 沿箭头多跳遍历子图  (靠"连线")
```
...
```
原文："苹果公司的 CEO 是库克，苹果总部位于库比蒂诺。"
       │  让 LLM 当"信息抽取器"，抽出 (实体, 关系, 实体)
       ▼
   (苹果公司, CEO是, 库克)
   (苹果公司, 总部位于, 库比蒂诺)
       │  存入图数据库 → 节点是实体，边是关系
       ▼
   [库克] ←CEO是— [苹果公司] —总部位于→ [库比蒂诺]
```
...
```
第1跳：识别实体 → [苹果公司]
第2跳：沿边走  → [苹果公司] —CEO是→ [库克]
第3跳：继续走  → [库克] —居住于→ [城市X] —特点→ ...
        ▼
   把这条路径(子图 + 关联原文)作为"带结构的证据链"喂给 LLM
```
