...
|---|---|---|---|---|
| **Semantic** 语义 ⭐ | 事实/知识("Jim 是我朋友") | Agent 主动调工具读写,影响工具使用 | Hot Path(agent 调 manage/search memory) | 向量索引 Store |
| **Episodic** 情景 | 过往案例(输入+期望输出) | few-shot 注入,让 agent 模仿历史决策 | Background(triage 代码自动向量检索注入) | Store 里的 examples namespace |
| **Procedural** 程序 | 规则/指令(系统 prompt 本身) | 重写 prompt,让 agent 行为演化 | Background(LLM optimizer 按反馈改写) | Store 里的自然语言规则 |
...
- **多租户从第一天就上**:namespace 三层 `(app, user_id, type)` 隔离用户 + 区分记忆类型;靠 `config` 注入 `langgraph_user_id`。
...
Q4 环境?
├─ 原型 → InMemoryStore
└─ 生产/多用户 → Postgres/Redis + namespace (app,user_id,type)
```
...
| 多租户 SaaS | 上述任意 + namespace `(app,user_id,type)` 隔离 |
