...
Tool2Vec	为每个工具生成合成查询再 embed	~5ms	中高	1-2周	粗筛（比纯 embedding 好 27-30% Recall）
...
效果：比纯描述 embedding 高 27-30% Recall@K（ToolBench 数据集）
...
Red Hat 生产实践：Tool2Vec + DeBERTa 分类器并行作为 Stage 1，ToolRefiner 作为 Stage 2
...
缺点：100+ 工具描述放不进 context window（~40-60 个工具描述就 4K tokens）；需要先粗筛
5. Fine-tuned 小模型
在你的领域数据上微调：

Gorilla（ICLR 2024）：微调 LLaMA-7B，API 调用准确率超 GPT-4 20%
